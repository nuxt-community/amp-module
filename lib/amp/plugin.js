...
export default async function (ctx, inject) {
  ...
  const path = '<%= options.css %>'
  const cssText = await import('!!raw-loader!sass-loader!~/assets/styles/amp-custom.scss').then(m => m.default || m)
  ctx.app.head.style.push({ cssText, type: 'text/css', 'amp-custom': vueMetaMajor >= 2 ? true : undefined })
