...
function registerSSRHook (options) {
  ...
  this.nuxt.hook('vue-renderer:ssr:templateParams', (params) => {
    ...
    if (options.removeLayoutTags) {
      params.APP = params.APP
        .replace(/<div data-server-rendered="true" id="__nuxt">.*<div id="__layout">/m, '')
        .replace(/<\/div><\/div>$/, '')
    }
        
    //force amp-sidebar to resist below body tag
    const matchAmpSidebar = /<amp-sidebar[^>]*>.*?<\/\amp-sidebar.*?>/gi.exec(params.APP)
    if (matchAmpSidebar && matchAmpSidebar.length === 1) {
      params.APP =
        // sidebar html
        params.APP.substring(matchAmpSidebar.index, matchAmpSidebar.index + matchAmpSidebar[0].length) +
        // html before sidebar
        params.APP.substring(0, matchAmpSidebar.index) +
        // html after sidebar
        params.APP.substring(matchAmpSidebar.index + matchAmpSidebar[0].length);
    }
